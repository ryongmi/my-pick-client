...
const PLATFORM_OPTIONS = [
  ...
  { type: 'twitter' as const, label: 'Twitter', icon: Twitter, color: 'text-blue-500' },
  { type: 'instagram' as const, label: 'Instagram', icon: Instagram, color: 'text-pink-500' },
  { type: 'tiktok' as const, label: 'TikTok', icon: ExternalLink, color: 'text-purple-500' },
];
...
export function CreatorAddModal({ isOpen, onClose, onSubmit }: CreatorAddModalProps) {
  const [formData, setFormData] = useState<CreatorFormData>({
    ...
    displayName: '',
    description: '',
    platforms: [{ type: 'youtube', username: '', url: '' }],
  });
  ...
  const addPlatform = () => {
    setFormData(prev => ({
      ...prev,
      platforms: [...prev.platforms, { type: 'youtube', username: '', url: '' }],
    }));
  };
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  {formData.platforms.map((platform, index) => {
                    ...
                    const platformOption = PLATFORM_OPTIONS.find(p => p.type === platform.type);
                    
                    return (
                      <div key={index} className="border rounded-lg p-4 bg-gray-50">
                        <div className="flex justify-between items-center mb-3">
                          <div className="flex items-center gap-2">
                            <PlatformIcon className={`h-5 w-5 ${platformOption?.color || 'text-gray-500'}`} />
